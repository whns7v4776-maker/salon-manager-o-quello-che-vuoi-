...
- Frontend Expo:
  ...
  - Sync token su backend tramite RPC `upsert_push_device`
- Backend Supabase:
  - Tabelle:
    - `public.push_devices`
...
2. L'app tenta la sincronizzazione token su Supabase (se sessione auth presente e workspace id UUID).
...
- La sync backend richiede sessione Supabase autenticata (`auth.uid()`) e `workspace_id` valido UUID.
